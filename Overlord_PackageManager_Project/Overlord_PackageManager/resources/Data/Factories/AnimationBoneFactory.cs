...
using Overlord_PackageManager.resources.Data.EntryTypes.Asset.Animation.KeyFrameTracks;
...
namespace Overlord_PackageManager.resources.Data.Factories
{
    ...
    {
        public static Entry CreateUnusedKeyframeTrack1(BinaryReader reader, uint id, uint relOffset)
        {
            return id switch
            {
                20 => new ScalarEntry<float>(id, relOffset, BinaryTypes.Float),                 // Track's time scale
                // Add more IDs here
                _ => new RawArrayEntry<byte>(id, relOffset, BinaryTypes.Byte)                   // Unknown entry
            };
        }

        public static Entry CreateUnusedKeyframeTrack2(BinaryReader reader, uint id, uint relOffset)
        {
            return id switch
            {
                20 => new ScalarEntry<float>(id, relOffset, BinaryTypes.Float),                 // Track's time scale
        ...
        public static Entry CreateBoneAnimationAsset(BinaryReader reader, uint id, uint relOffset)
        {
            return id switch
            {
                ...
                21 => new ScalarEntry<ulong>(id, relOffset, BinaryTypes.UInt64),                // Total Time in microseconds
                22 => new UnusedKeyframeTrack1(id, relOffset),                                  // Unkown Keyframe data
                23 => new UnusedKeyframeTrack2(id, relOffset),                                  // Unkown Keyframe data
