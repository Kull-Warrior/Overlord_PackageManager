...
{
    class BoneAnimationSubTableType25(uint id, uint relOffset) : Entry(id, relOffset)
    {
        ...
        public void Read(BinaryReader reader, long origin, Func<uint, uint, Entry> entryFactory)
        {
            ...
            foreach (var entry in varRefTable.Entries)
            {
                ...
                {
                    ((BoneAnimationSubTableType25SubTableType21)entry).Read(reader, varRefTable.origin, BoneAnimationSubTableType25SubTableType21Dictionary);
                }
            }
        }
