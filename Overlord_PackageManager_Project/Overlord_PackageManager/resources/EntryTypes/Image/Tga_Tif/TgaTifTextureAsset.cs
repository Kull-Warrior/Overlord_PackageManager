    ...
    class TgaTifTextureAsset(uint id, uint relOffset) : Entry(id, relOffset), IHasReferenceTable
    {
        ...
        public void Read(BinaryReader reader, long origin, Func<uint, uint, Entry> entryFactory)
        {
            ...
            foreach (var entry in Table.Entries)
            {
                ...
                {
                    ((TgaTifTextureData)entry).Read(reader, Table.PayloadStartOffset, RawTgaTifTextureDataDictionary);
                }
            }
        }
        ...
        public void WriteToFile(string baseDir)
        {
            ...
            byte[] rawRGBA = ((BlobEntry)dataContainer.Table.Entries[0]).varBytes;
