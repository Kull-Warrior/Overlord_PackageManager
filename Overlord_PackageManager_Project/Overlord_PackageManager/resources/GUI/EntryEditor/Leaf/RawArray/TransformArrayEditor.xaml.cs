...
{
    /// <summary>
    /// View model for a single Transform item in the array
    /// </summary>
    public class TransformArrayItem
    {
        public TransformEditor? Editor { get; set; }
        public ObservableTransform ObservableValue { get; set; } = null!;
    }

    /// <summary>
    /// Interaction logic for UInt16ArrayEditor.xaml
    /// </summary>
    public partial class TransformArrayEditor : UserControl, IValueEditor
    {
        private ObservableValue<Transform[]>? _observableArray;
        private ObservableCollection<TransformArrayItem> _items = new();
        private bool _isUpdating;

        public TransformArrayEditor()
        {
            InitializeComponent();
            ArrayItemsControl.ItemsSource = _items;
        }

        public TransformArrayEditor(ObservableValue<Transform[]> array) : this()
        {
            BindToArray(array);
        }
        ...
        private void BindToArray(ObservableValue<Transform[]> array)
        {
        ...
        {
            if (!_isUpdating && e.PropertyName == nameof(ObservableValue<Transform[]>.Value))
            {
                _isUpdating = true;
        ...
        private void RebuildItems(Transform[] values)
        {
            _items.Clear();

            for (int i = 0; i < values.Length; i++)
            {
                ObservableTransform observableValue = new ObservableTransform(values[i]);

                // When this individual value changes, update the full array
                observableValue.PropertyChanged += (s, e) =>
                {
                    if (e.PropertyName == nameof(ObservableTransform.Value))
                    {
                        UpdateArrayFromItems();
                    }
                };

                TransformEditor editor = new TransformEditor(observableValue)
                {
                    Label = $"[{i}]"
                };

                TransformArrayItem item = new TransformArrayItem
                {
                    Editor = editor,
        ...
        private void AddButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            Transform[] newArray = new Transform[_observableArray.Value.Length + 1];
        ...
        {
            if (sender is Button button && button.Tag is TransformArrayItem item)
            {
                _items.Remove(item);
                UpdateArrayFromItems();
                UpdateCountDisplay();
